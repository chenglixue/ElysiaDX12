...
namespace ElysiaCore
{
                                 ...
                                 bool isFlush)
    {
        ...
        {
            D3D12_RESOURCE_STATES oldState = resource.GetUsageState(i);
            if (oldState != newStates[i])
            {
                D3D12_RESOURCE_BARRIER& barrierDesc = m_resourceBarriers[m_numQueuedBarriers];
                m_numQueuedBarriers ++;
                                 ...
                                 bool isFlush)
    {
        ...
        auto resource = RT->GetTexture();
        auto subresourceIndex = UINT(RT->GetSubresourceIndex(mipmapNum));

        D3D12_RESOURCE_STATES oldState = resource->GetUsageState(mipmapNum);
        if (oldState != newState)
        {
            D3D12_RESOURCE_BARRIER& barrierDesc = m_resourceBarriers[m_numQueuedBarriers];
            m_numQueuedBarriers ++;
