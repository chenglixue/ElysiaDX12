...
namespace ElysiaCore
{
    ...
    void DX12RootParameter::InitAsConstants(UINT num32BitValues,
                                            UINT slotIndex,
                                            UINT space,
    ...
    void DX12RootParameter::InitAsConstantBufferView(UINT slotIndex,
                                                     D3D12_SHADER_VISIBILITY shaderVisibility,
                                                     UINT Space)
    {
    ...
    void DX12RootParameter::InitAsShaderResourceView(UINT shaderRegister,
                                                     UINT registerSpace,
                                                     D3D12_SHADER_VISIBILITY visibility)
    {
        m_rootParamter.ParameterType = D3D12_ROOT_PARAMETER_TYPE_SRV;
        m_rootParamter.ShaderVisibility = visibility;
        m_rootParamter.Descriptor.ShaderRegister = shaderRegister;
        m_rootParamter.Descriptor.RegisterSpace = registerSpace;

        m_rootParamter.Descriptor.Flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE;
    }
    ...
    /// <param name="shaderVisibility"></param>
    void DX12RootParameter::InitAsDescriptorTable(UINT rangeCount,
                                                  const D3D12_DESCRIPTOR_RANGE1* descriptorRangeData,
                                                  D3D12_SHADER_VISIBILITY shaderVisibility)
    {
    ...
    /// <returns></returns>
    void DX12RootParameter::SetTableRange(D3D12_DESCRIPTOR_RANGE_TYPE rangeType,
                                          UINT numDescriptors,
                                          UINT slotIndex,
                                          UINT rangeIndex,
    ...
    void DX12RootSignature::InitStaticSamplers(UINT slotIndex,
    ...
    void DX12RootSignature::Init(ID3D12Device* device, D3D12_ROOT_SIGNATURE_FLAGS flags)
    {
        ...
        ElysiaHelper::ThrowIfFailed(device->CreateRootSignature(0,
