...
{
    DX12StagingDescriptorHeap::DX12StagingDescriptorHeap(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType,
                                                         UINT numDescriptor) :
    ...
    DX12StagingDescriptorHeap::~DX12StagingDescriptorHeap()
    {
        ...
        {
            ElysiaHelper::ThrowRuntimeError(
                "There were active handles when the descriptor heap was destroyed. Look for leaks.");
        }
