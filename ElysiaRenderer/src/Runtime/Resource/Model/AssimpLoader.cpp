...
void LoadMaterials(const aiScene* pScene, LoadedModel& model)
{
         ...
         materialIndex ++)
    {
        ...
        {
            loadTexturePath(aiTextureType_DIFFUSE, material.textureNames[static_cast<UINT64>(MaterialTextureType::Albedo)]);
        }
        else
        {
            loadTexturePath(aiTextureType_BASE_COLOR, material.textureNames[static_cast<UINT64>(MaterialTextureType::Albedo)]);
        }
        if (aiMaterial->GetTextureCount(aiTextureType_NORMALS) > 0)
        {
            loadTexturePath(aiTextureType_NORMALS,material.textureNames[static_cast<UINT64>(MaterialTextureType::Normal)]);
        }
        else
        {
            loadTexturePath(aiTextureType_HEIGHT,material.textureNames[static_cast<UINT64>(MaterialTextureType::Normal)]);
        }

        loadTexturePath(aiTextureType_EMISSIVE,material.textureNames[static_cast<UINT64>(MaterialTextureType::Emissive)]);
        loadTexturePath(aiTextureType_METALNESS,material.textureNames[static_cast<UINT64>(MaterialTextureType::Metallic)]);
        loadTexturePath(aiTextureType_DIFFUSE_ROUGHNESS,material.textureNames[static_cast<UINT64>(MaterialTextureType::Roughness)]);
                          ...
                          GrowableList<MaterialTexture*>& materialTextures)
{
    ...
    for (UINT64 matIdx = 0; matIdx < numMaterials; matIdx++)
    {
        ...
        for (UINT64 texType = 0;texType < static_cast<UINT64>(MaterialTextureType::Count); texType++)
        {
            material.textures[texType] = ElysiaRenderer::TextureManager::Handle::Invalid();

            std::wstring path = fileDirectory;

            if (material.textureNames[texType].length() <= 0 || FileExists(path + material.textureNames[texType]) == false)
            {
                bool hasTex = false;
                switch (static_cast<MaterialTextureType>(texType))
                {
                    ...
                    {
                        if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Normal" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Normal" + L".png";
                            hasTex = true;
                        }
                        else if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_normal" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_normal" + L".png";
                            hasTex = true;
                        }
                    ...
                    {
                        if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Metallic" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Metallic" + L".png";
                            hasTex = true;
                        }
                        else if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_metallic" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_metallic" + L".png";
                            hasTex = true;
                        }
                    ...
                    {
                        if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Roughness" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_Roughness" + L".png";
                            hasTex = true;
                        }
                        else if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_roughness" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[UINT64(MaterialTextureType::Albedo)]) + L"_roughness" + L".png";
                            hasTex = true;
                        }
                    ...
                    {
                        if (FileExists(path + RemoveLastUnderscoreAndAfter(material.textureNames[texType]) + L"_AO" + L".png"))
                        {
                            path += RemoveLastUnderscoreAndAfter(material.textureNames[texType]) + L"_AO" + L".png";
                            hasTex = true;
                        }
                        break;
                    }
                ...
                if (!hasTex)
                {
                    if (texType == static_cast<UINT64>(MaterialTextureType::Albedo) || texType == static_cast<UINT64>(MaterialTextureType::Roughness)
                        || texType == static_cast<UINT64>(MaterialTextureType::Occlusion))
                    {
                        path += ElysiaRenderer::DefaultWhiteTexturePath;
                    }
                    else if (texType == static_cast<UINT64>(MaterialTextureType::Height) || texType == static_cast<UINT64>(MaterialTextureType::Emissive)
                             || texType == static_cast<UINT64>(MaterialTextureType::Metallic) || texType == static_cast<UINT64>(MaterialTextureType::Specular))
                    {
...
void LoadMeshData(const std::wstring& filePath, const aiScene* pScene,
                  float sceneScale, LoadedModel& model)
{
...
void LoadedModel::Mesh::InitCommon(uint64 vbAddress, uint64 ibAddress,
                                   uint64 vtxOffset_, uint64 idxOffset_)
{
...
bool LoadModel(const std::wstring& filePath, bool bInvertTexcoordY,
               bool bImportMeshes,
