...
namespace ElysiaRenderer
{
    ...
    TextureManager::Handle TextureManager::LoadDynamicTexture(
    ...
    TextureManager::Handle TextureManager::LoadResidentTexture(
        const std::wstring& filePath,
        bool isSRGB)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    TextureManager::Handle TextureManager::CreateTexture(const D3D12_RESOURCE_DESC& resourceDesc,
                                                         TexTypeFlags flag,
                                                         std::wstring name)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    int TextureManager::GetReferenceCount(const std::wstring& filePath)
    {
        ...
        return m_dynamicTextureMap.contains(nameHash)
                   ? m_dynamicTextureMap.at(nameHash)->refCount.load()
                   : 0;
    }
    ...
    void TextureManager::PrintStats()
    {
        ...
        std::wcout << L"[TextureManager] Total Managed Textures: " << m_dynamicTextureMap.size() <<
            std::endl;
