...
namespace ElysiaRenderer
{
	...
	std::unique_ptr<DX12Shader>					DX12Device::CreateShader(ShaderCreateDesc& shaderCreateDesc)
	{
		...
		auto pdbName = (shaderCreateDesc.shaderName + shaderCreateDesc.entryPoint + L"myshader");
		 
		LPCWSTR pszArgs[] =
		{
			shaderCreateDesc.shaderName.c_str(),            // Optional shader source file name for error reporting and for PIX shader source view.  
			L"-E", shaderCreateDesc.entryPoint.c_str(),     // Entry point.
			L"-T", target.c_str(),							// Target.
			L"-D", L"MYDEFINE=1",							// A single define.
			L"-D", L"EDITOR"
			//L"-Fo", (shaderCreateDesc.shaderName + shaderCreateDesc.entryPoint + L".bin").c_str(),				// Optional. Stored in the pdb. 
			//L"-Fd", (shaderCreateDesc.shaderName + shaderCreateDesc.entryPoint + L".pdb").c_str(),				// The file name of the pdb. This must either be supplied or the autogenerated file name must be used.
