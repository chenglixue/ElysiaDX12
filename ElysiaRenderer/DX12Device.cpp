...
namespace ElysiaRenderer
{
	...
	std::unique_ptr<DX12Shader>					DX12Device::CreateShader(ShaderCreateDesc& shaderCreateDesc)
	{
		...
		std::vector<LPCWSTR> pszArgs =
		{
			...
			//L"-Fo", binName,								// Optional. Stored in the pdb. 
			//L"-Fd", pdbName,								// The file name of the pdb. This must either be supplied or the autogenerated file name must be used.
		...
		switch (usetData.shadowQuality)
		{
			...
			{
				pszArgs.emplace_back(L"SHADOW_QUALITY_LOW");
				break;
			}
			case ShadowQuality::Middle:
			{
				pszArgs.emplace_back(L"SHADOW_QUALITY_MIDDLE");
				break;
			}
			case ShadowQuality::High:
			{
				pszArgs.emplace_back(L"SHADOW_QUALITY_HIGH");
				break;
			}
