...
{
    float2 offset = 2.0 * samplePoint - 1.0;
    ...
    {
        r = offset.x;
        phi = (3.14159265f / 4.0f) * (offset.y / offset.x);
    }
    else
    {
        r = offset.y;
        phi = (3.14159265f / 2.0f) - (3.14159265f / 4.0f) * (offset.x / offset.y);
    }
                      ...
                      uint CSMIndex = 0)
{
    ...
    {
        float2 baseSobol = sobolSequence[i];
        ...
        float sampleShadow = shadowTex.Sample(pcfSampler, sampleUV);
        shadow += step(lightDepth, sampleShadow);
    }
...
inline float ManualSobelPCF(float4 screenSize,
                            float2 screenUV,
                            float2 shadowPos,
                            float lightDepth,
                            Texture2D shadowTex,
