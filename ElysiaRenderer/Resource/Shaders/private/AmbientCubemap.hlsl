...
float3 PrefilterEnvMap(uint2 Random, float Roughness, float3 R)
{
    ...
    for (uint i = 0; i < NumSamples; i++)
    {
        ...
        if (NoL > 0)
        {
            FilteredColor += g_SkyboxTex.SampleLevel(g_Sampler_WarpU_WarpV_Linear, L, 0).rgb * NoL;
            Weight += NoL;
        }
    }
...
float3 DiffuseIBL(uint2 Random, float3 DiffuseColor, float Roughness, float3 N, float3 KD)
{
    ...
    DiffuseLighting += g_SkyboxTex.SampleLevel(g_Sampler_WarpU_WarpV_Linear, N, 5) * KD;
...
float3 SpecularIBL(uint2 Random, float3 SpecularColor, float Roughness, float3 N, float3 V)
{
    ...
    float mipmapLevel = ComputeCubemapMipFromRoughness(Roughness, 10);
    
    float3 R = 2 * dot(V, N) * N - V;
    float3 PrefilteredColor = g_SkyboxTex.SampleLevel(g_Sampler_WarpU_WarpV_Linear, R, mipmapLevel);
